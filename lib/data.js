  ...
  const members = [
    ...
    ...memberEmails
      ...
      .map((email) => {
        ...
        const isActiveMember = organizationMember?.status === "active";
        return {
          email,
          role: "member",
          status: isActiveMember ? "active" : "invited",
          notificationPreference: "immediate",
          invitedAt: isActiveMember ? null : createdAt,
          joinedAt: isActiveMember ? createdAt : null,
          removedAt: null,
          removedByEmail: "",
          removedByName: "",
          history: [
            buildMembershipHistoryEntry({
              type: isActiveMember ? "joined" : "invited",
              at: createdAt,
              byEmail: ownerEmail,
              byName: ensureString(input.ownerName, "Workspace owner"),
              role: "member"
...
export async function addWorkspaceMember(input) {
  ...
  const db = await getDatabase();
  const workspacesCollection = db.collection("workspaces");
  const nextStatus = organizationMember.status === "active" ? "active" : "invited";
  const nextHistoryType = nextStatus === "active" ? "joined" : "invited";
  if (existingMember && !isCurrentMembershipStatus(existingMember.status)) {
    await workspacesCollection.updateOne(
      { slug: input.workspaceSlug, "members.email": memberEmail },
      {
        $set: {
          ...
          "members.$.invitedAt": nextStatus === "invited" ? changeAt : null,
          "members.$.joinedAt": nextStatus === "active" ? changeAt : null,
          "members.$.removedAt": null,
          "members.$.removedByEmail": "",
          "members.$.removedByName": "",
          "members.$.history": appendMembershipHistory(existingMember.history, {
      ...
      { slug: input.workspaceSlug },
      {
        $push: {
          members: {
            ...
            status: nextStatus,
            notificationPreference: "immediate",
            invitedAt: nextStatus === "invited" ? changeAt : null,
            joinedAt: nextStatus === "active" ? changeAt : null,
            removedAt: null,
            removedByEmail: "",
            removedByName: "",
            history: [
