...
function normalizeMembershipStatus(status) {
  if (status === "active" || status === "declined" || status === "removed") {
    return status;
  }
...
function parseMembershipHistory(value) {
  ...
  return rawEvents
    .map((item) => {
      ...
      const at = normalizeDateString(item.at);
      if (!type || !at) {
        return null;
      }
      ...
      return {
        type,
        at,
        byEmail: normalizeEmail(item.byEmail),
        byName: ensureString(item.byName),
...
function buildMembershipHistoryEntry({ type, at = new Date(), byEmail = "", byName = "", role = "" }) {
  return {
    type: ensureString(type),
    at: normalizeDateString(at) ?? new Date().toISOString(),
    byEmail: normalizeEmail(byEmail),
    byName: ensureString(byName),
    role: ensureString(role)
  };
}

function appendMembershipHistory(history, entry) {
  return [...parseMembershipHistory(history), buildMembershipHistoryEntry(entry)];
}
  ...
  const members = rawMembers
    .map((member) => {
      ...
      return {
        ...
        invitedAt: normalizeDateString(item.invitedAt),
        joinedAt: normalizeDateString(item.joinedAt),
        removedAt: normalizeDateString(item.removedAt),
        removedByEmail: normalizeEmail(item.removedByEmail),
        removedByName: ensureString(item.removedByName),
        history: parseMembershipHistory(item.history)
      };
    })
    .filter(Boolean);
  ...
  const members = rawMembers
    .map((member) => {
      ...
      return {
        ...
        role: item.role === "owner" || item.role === "admin" ? item.role : "member",
        status: normalizeMembershipStatus(item.status),
        notificationPreference: normalizeWorkspaceNotificationPreference(item.notificationPreference),
        invitedAt: normalizeDateString(item.invitedAt),
        joinedAt: normalizeDateString(item.joinedAt),
        removedAt: normalizeDateString(item.removedAt),
        removedByEmail: normalizeEmail(item.removedByEmail),
...
function mapOrganization(doc) {
  ...
  const members = parseOrganizationMembers(doc.members, ownerEmail);
  const currentMembers = members.filter((member) => isCurrentMembershipStatus(member.status));
  const activeMembers = members.filter((member) => member.status === "active");
  const memberHistory = members.filter((member) => member.status === "removed");
...
function mapWorkspace(doc) {
  const ownerEmail = ensureString(doc.ownerEmail, "owner@example.com");
  const members = parseWorkspaceMembers(doc.members, ownerEmail);
  const currentMembers = members.filter((member) => isCurrentMembershipStatus(member.status));
  const activeMembers = members.filter((member) => member.status === "active");
  const memberHistory = members.filter((member) => member.status === "removed");
...
function getTodayTasks(tasks) {
  ...
  return tasks
    .filter((task) => task.status !== "done" && task.dueDate && task.dueDate.slice(0, 10) <= todayDate)
    .sort((a, b) => {
...
function mapWorkspaceChatMessage(doc) {
  return {
    ...
    sources: Array.isArray(doc.sources) ? doc.sources.map((item) => normalizeChatSource(item)).filter((item) => Boolean(item.label)).slice(0, 8) : [],
...
function normalizeChatSource(input) {
  if (typeof input === "string") {
    const label = ensureString(input);
    return {
      label,
      sourceType: "",
      sourceId: "",
  ...
  return {
    label: ensureString(input.label),
    sourceType: ensureString(input.sourceType),
    sourceId: ensureString(input.sourceId),
    href: ensureString(input.href)
  };
}
...
function canManageWorkspaceMembers(workspace, organization, userEmail) {
  ...
  if (organizationRole === "owner" || workspace.ownerEmail === normalizedUserEmail) {
    return true;
  }

  const workspaceMember = getWorkspaceMemberRecord(workspace, normalizedUserEmail);
  return Boolean(workspaceMember && workspaceMember.status === "active" && workspaceMember.role === "admin");
}
...
function canViewAiPrivateWorkspaceInputs(organization, userEmail) {
  return getOrganizationRole(organization, userEmail) === "owner";
}
...
async function getAuthUserNamesByEmails(emails) {
  const normalizedEmails = uniqueStrings((Array.isArray(emails) ? emails : []).map((email) => normalizeEmail(email)).filter(Boolean));
  ...
  for (const doc of docs) {
    ...
    const name = ensureString(doc?.name);
    if (email && name) {
...
function decorateMembersWithNames(members, ownerEmail, ownerName, nameDirectory) {
  return (Array.isArray(members) ? members : []).map((member) => {
    const email = normalizeEmail(member?.email);
    const memberName = ensureString(member?.name);
    const resolvedName = memberName || nameDirectory.get(email) || (email === normalizeEmail(ownerEmail) ? ensureString(ownerName) : "");
    return {
      ...member,
      name: resolvedName
    };
    ...
    members: [
      {
        ...
        history: [
          buildMembershipHistoryEntry({
            type: "joined",
            at: createdAt,
            byEmail: normalizedOwnerEmail,
            byName: ensureString(input.ownerName, "Organization owner"),
            role: "owner"
          })
        ]
      }
  ...
  await organizationsCollection.updateOne(
    { slug: input.organizationSlug, "members.email": normalizedUserEmail },
    {
      $set: {
        ...
        "members.$.removedAt": null,
        "members.$.removedByEmail": "",
        "members.$.removedByName": "",
        "members.$.history": appendMembershipHistory(member.history, {
          type: "joined",
          at: joinedAt,
          byEmail: normalizedUserEmail,
          role: member.role
        })
      }
    }
...
export async function acceptWorkspaceInvite(input) {
  ...
  if (workspaceMember.status !== "invited") {
    throw new Error("Invite no longer available");
  }
  ...
  await workspacesCollection.updateOne(
    { slug: input.workspaceSlug, "members.email": normalizedUserEmail },
    {
      $set: {
        ...
        "members.$.removedByEmail": "",
        "members.$.removedByName": "",
        "members.$.history": appendMembershipHistory(workspaceMember.history, {
          type: "joined",
          at: joinedAt,
          byEmail: normalizedUserEmail,
...
export async function rejectOrganizationInvite(input) {
  const organization = await getOrganizationBySlug(input.organizationSlug);
  ...
  return { message: "Organization invite declined." };
}
...
export async function rejectWorkspaceInvite(input) {
  ...
  return { message: "Workspace invite declined." };
}
export async function getWorkspaceDashboardData(userEmail, userName, organizationSlug = "") {
  if (!userEmail) {
    return {
      ...
      permissions: {
        ...
        workspaceNotificationPreference: "immediate"
      },
      accessGate: null,
      pendingOrganizationInvites: [],
      pendingWorkspaceInvites: []
  ...
  if (!organization) {
    ...
    return {
      ...
      pendingOrganizationInvites: baseAccessGate.pendingOrganizationInvites,
      pendingWorkspaceInvites: []
    };
  }
  ...
  const canSeeAiPrivate = canViewAiPrivateWorkspaceInputs(organization, userEmail);
  const files = fileDocs
    .map((doc) => mapWorkspaceFile(doc))
    .filter((file) => canSeeAiPrivate || !file.aiPrivate);
  const updates = updateDocs
    .map((doc) => mapWorkspaceUpdate(doc))
    .filter((update) => canSeeAiPrivate || !update.aiPrivate);
  ...
  const visibleTasks = tasks.filter((task) => visibleSlugs.has(task.workspaceSlug));
  ...
  return {
    ...
    todayTasks: getTodayTasks(visibleTasks),
...
export async function getWorkspaceDetailData(slug, userEmail) {
  ...
  const canonicalWorkspaceSlug = workspace.slug;
  const [fileDocs, updateDocs, taskDocs] = await Promise.all([
    filesCollection.find({ workspaceSlug: canonicalWorkspaceSlug }).sort({ createdAt: -1 }).limit(20).toArray(),
    updatesCollection.find({ workspaceSlug: canonicalWorkspaceSlug }).sort({ createdAt: -1 }).limit(20).toArray(),
    tasksCollection.find({ workspaceSlug: canonicalWorkspaceSlug }).sort({ createdAt: -1 }).limit(50).toArray()
  ]);
  ...
  const nameDirectory = await getAuthUserNamesByEmails(memberEmails);
  const organizationMembersWithNames = decorateMembersWithNames(
    organization.members,
    organization.ownerEmail,
    organization.ownerName,
    nameDirectory
  );
  const workspaceMembersWithNames = decorateMembersWithNames(
    workspace.members,
    workspace.ownerEmail,
    workspace.ownerName,
    nameDirectory
  );
  const organizationWithNames = {
    ...organization,
    members: organizationMembersWithNames,
    currentMembers: organizationMembersWithNames.filter((member) => isCurrentMembershipStatus(member.status)),
    activeMembers: organizationMembersWithNames.filter((member) => member.status === "active"),
    memberHistory: organizationMembersWithNames.filter((member) => member.status === "removed")
  };
  const workspaceWithNames = {
    ...workspace,
    members: workspaceMembersWithNames,
    currentMembers: workspaceMembersWithNames.filter((member) => isCurrentMembershipStatus(member.status)),
    activeMembers: workspaceMembersWithNames.filter((member) => member.status === "active"),
    memberHistory: workspaceMembersWithNames.filter((member) => member.status === "removed")
  };

  return {
    organization: organizationWithNames,
    workspace: workspaceWithNames,
    files,
    updates,
    tasks,
    knowledgeSummary: {
      ...(workspaceWithNames.knowledgeSummary ?? buildKnowledgeSummaryFallback({ workspace: workspaceWithNames, files, updates, tasks })),
    ...
    permissions: {
      ...
      workspaceNotificationPreference: getWorkspaceMemberRecord(workspace, userEmail)?.notificationPreference ?? "immediate"
    }
  };
}
...
export async function createWorkspace(input) {
  ...
  const organizationMemberEmails = new Set(
    organization.members
      .filter((member) => isCurrentMembershipStatus(member.status))
  ...
  const members = [
    {
      ...
      history: [
        buildMembershipHistoryEntry({
          ...
          at: createdAt,
          byEmail: ownerEmail,
          byName: ensureString(input.ownerName, "Workspace owner"),
          role: "owner"
        })
      ]
    },
    ...memberEmails
      ...
      .map((email) => {
        ...
        const memberRole = organizationMember?.role === "admin" ? "admin" : "member";
        return {
          email,
          role: memberRole,
          status: "invited",
...
export async function addOrganizationMember(input) {
  ...
  const requesterName = ensureString(input.requesterName);

  if (!memberEmail) {
    throw new Error("Member email is required");
  }

  const existingMember = organization.members.find((member) => member.email === memberEmail);
  if (existingMember && isCurrentMembershipStatus(existingMember.status)) {
    return organization;
  }
  ...
  if (existingMember && !isCurrentMembershipStatus(existingMember.status)) {
    await organizationsCollection.updateOne(
      { slug: organization.slug, "members.email": memberEmail },
      {
        $set: {
          "members.$.role": requestedRole,
          "members.$.status": "invited",
          "members.$.invitedAt": invitedAt,
          "members.$.joinedAt": null,
          "members.$.removedAt": null,
          "members.$.removedByEmail": "",
          "members.$.removedByName": "",
          "members.$.history": appendMembershipHistory(existingMember.history, {
            type: "invited",
            at: invitedAt,
            byEmail: input.requesterEmail,
...
export async function saveWorkspaceFile(input) {
  ...
  const document = {
    ...input,
    workspaceSlug: workspace.slug,
    workspaceName: ensureString(input.workspaceName, workspace.name),
...
export async function getAuthorizedWorkspaceFileById(fileId, userEmail) {
  ...
  const file = mapWorkspaceFile(doc);
  const workspace = await getAuthorizedWorkspace(file.workspaceSlug, userEmail);
  if (!workspace) {
    return null;
  }
  ...
  if (file.aiPrivate) {
    const organization = await getOrganizationBySlug(workspace.organizationSlug);
    if (!organization || !canViewAiPrivateWorkspaceInputs(organization, userEmail)) {
      return null;
    }
  }
...
export async function addWorkspaceMember(input) {
  ...
  if (!canManageWorkspaceMembers(workspace, organization, input.requesterEmail)) {
    throw new Error("Only workspace owners/admins or organization owners can manage workspace access");
  }
  ...
  const existingMember = workspace.members.find((member) => member.email === memberEmail);
  if (existingMember && isCurrentMembershipStatus(existingMember.status)) {
    return workspace;
  }

  const organizationMember = organization.members.find((member) => member.email === memberEmail && isCurrentMembershipStatus(member.status));
  if (!organizationMember) {
  ...
  const db = await getDatabase();
  const workspacesCollection = db.collection("workspaces");
  const nextStatus = "invited";
  const nextHistoryType = "invited";
  const nextRole = organizationMember.role === "admin" ? "admin" : "member";
  if (existingMember && !isCurrentMembershipStatus(existingMember.status)) {
    await workspacesCollection.updateOne(
      { slug: input.workspaceSlug, "members.email": memberEmail },
      {
        $set: {
          "members.$.role": nextRole,
          "members.$.status": nextStatus,
          "members.$.notificationPreference": "immediate",
          "members.$.invitedAt": changeAt,
          "members.$.joinedAt": null,
          "members.$.removedAt": null,
          "members.$.removedByEmail": "",
          "members.$.removedByName": "",
          "members.$.history": appendMembershipHistory(existingMember.history, {
            type: nextHistoryType,
            at: changeAt,
            byEmail: input.requesterEmail,
            byName: requesterName,
            role: nextRole
...
export async function removeWorkspaceMember(input) {
  ...
  const member = getWorkspaceMemberRecord(workspace, memberEmail);
  if (!member || !isCurrentMembershipStatus(member.status)) {
  ...
  await workspacesCollection.updateOne(
    { slug: input.workspaceSlug, "members.email": memberEmail },
    {
      $set: {
        ...
        "members.$.removedAt": removedAt,
        "members.$.removedByEmail": normalizeEmail(input.requesterEmail),
        "members.$.removedByName": ensureString(input.requesterName),
        "members.$.history": appendMembershipHistory(member.history, {
          type: "removed",
          at: removedAt,
          byEmail: input.requesterEmail,
          byName: input.requesterName,
          role: member.role
        })
      }
    }
...
export async function removeOrganizationMember(input) {
  ...
  if (memberEmail === organization.ownerEmail) {
    throw new Error("The organization owner cannot be removed");
  }

  const member = getOrganizationMemberRecord(organization, memberEmail);
  if (!member || !isCurrentMembershipStatus(member.status)) {
  ...
  const db = await getDatabase();
  const workspacesCollection = db.collection("workspaces");
  const workspaceOwnerDocs = await workspacesCollection
    .find({
      organizationSlug: organization.slug,
  ...
  if (workspaceOwnerDocs.length > 0) {
    const ownedWorkspaceNames = workspaceOwnerDocs.map((doc) => ensureString(doc.name, "Untitled workspace")).join(", ");
    throw new Error(`Transfer or delete this member's workspace ownership first: ${ownedWorkspaceNames}`);
  }
  ...
  const affectedWorkspaces = await workspacesCollection
    .find({
      organizationSlug: organization.slug,
  ...
  for (const workspaceDoc of affectedWorkspaces) {
    const workspace = mapWorkspace(workspaceDoc);
    const workspaceMember = getWorkspaceMemberRecord(workspace, memberEmail);
    if (!workspaceMember || !isCurrentMembershipStatus(workspaceMember.status)) {
      continue;
    }

    await workspacesCollection.updateOne(
      { slug: workspace.slug, "members.email": memberEmail },
      {
        $set: {
          "members.$.status": "removed",
          "members.$.removedAt": removedAt,
...
export async function updateWorkspaceMemberNotificationPreference(input) {
  const workspace = await getWorkspaceBySlug(input.workspaceSlug);
  if (!workspace) {
    throw new Error("Workspace not found");
  }
  ...
  const organization = await getOrganizationBySlug(workspace.organizationSlug);
  if (!organization || !hasWorkspaceAccess(workspace, organization, input.userEmail)) {
  ...
  const notificationPreference = normalizeWorkspaceNotificationPreference(input.notificationPreference);

  const db = await getDatabase();
  const workspacesCollection = db.collection("workspaces");
  const result = await workspacesCollection.updateOne(
    { slug: input.workspaceSlug, "members.email": normalizedUserEmail },
    { $set: { "members.$.notificationPreference": notificationPreference } }
  );

  if (!result.matchedCount && workspace.ownerEmail === normalizedUserEmail) {
    await workspacesCollection.updateOne(
      { slug: input.workspaceSlug },
      {
        $push: {
  ...
  return { workspaceSlug: workspace.slug, notificationPreference };
}
...
export async function getWorkspaceChatContext(slug, userEmail, question) {
  ...
  const canonicalWorkspaceSlug = data.workspace.slug;
  const aiVisible = getAiVisibleWorkspaceInputs(data);
  ...
  for (const chunk of retrievedChunks) {
    const sourceType = ensureString(chunk.sourceType);
    const sourceId = ensureString(chunk.sourceId);
    const label = ensureString(chunk.sourceLabel);
    if (!label) {
      continue;
    }
    ...
    let href = "";
    if (sourceType === "file" && sourceId) {
      href = `/api/workspace-files/${encodeURIComponent(sourceId)}/download`;
    }
    if (sourceType === "update" && sourceId) {
      href = `/dashboard/${encodeURIComponent(data.workspace.slug)}/updates`;
    }
...
export async function saveWorkspaceChatMessage(input) {
  ...
  const document = {
    ...
    sources: Array.isArray(input.sources)
      ? input.sources
          .map((item) => normalizeChatSource(item))
