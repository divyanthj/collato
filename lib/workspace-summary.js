...
export function buildKnowledgeSummaryFallback(data) {
  const aiVisibleFiles = data.files.filter((file) => !file.aiPrivate);
  const aiVisibleUpdates = data.updates.filter((update) => !update.aiPrivate);

  const meaningfulFiles = aiVisibleFiles.filter((file) => {
    const sourceText = file.extractedText || file.manualNotes || file.knowledgeText;
    return !isPlaceholderKnowledgeText(sourceText);
  });

  if (meaningfulFiles.length === 0 && aiVisibleUpdates.length === 0 && data.tasks.length === 0) {
    return {
...
function buildKnowledgeContext(data) {
  ...
  const fileContext = aiVisibleFiles
    .map((file) => {
      const sourceText = file.extractedText || file.manualNotes || file.knowledgeText;
...
Content: ${compactText(sourceText).slice(0, 6000)}`;
  ...
  const updateContext = aiVisibleUpdates
    .map(
      (update) =>
