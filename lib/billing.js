...
async function getSubscriptionsForOrganization(organizationSlug) {
  ...
  return docs.map(mapSubscriptionDoc);
}
...
export async function getBillingStatusForOrganization(organization) {
  ...
  if (normalizedOwnerEmail) {
    ...
    if (emailSubscriptions.length > 0) {
      ...
      for (const subscription of eligibleEmailSubscriptions) {
        if (!mergedBySubscriptionId.has(subscription.subscriptionId)) {
          mergedBySubscriptionId.set(subscription.subscriptionId, subscription);
        }
      }
  ...
  if (subscriptions.length === 0 && ownerFreeSeats > 0) {
  ...
  const primarySubscription = activeSubscriptions[0] ?? subscriptions[0];
  const hasMultipleActiveSubscriptions = activeSubscriptions.length > 1;
  const active = activeSubscriptions.length > 0;
  const quantity = activeSubscriptions.reduce((sum, subscription) => sum + Math.max(subscription.quantity, 0), 0);
  const remainingSeats = Math.max(quantity - usedSeats, 0);
  ...
  return {
    ...
    usedSeats,
    remainingSeats,
    planInterval: resolvePlanInterval(activeSubscriptions, primarySubscription.planInterval || null),
    status: active ? "active" : primarySubscription.status,
    migrationRequired: activeSubscriptions.some((subscription) => Boolean(subscription.migrationRequired)),
    isLegacyVariant: activeSubscriptions.some((subscription) => Boolean(subscription.isLegacyVariant)),
    ...
    customerEmail: primarySubscription.customerEmail || "",
...
export async function updateLemonSubscriptionFromApp({ subscriptionId, interval, quantityToAdd }) {
  const apiKey = process.env.LEMONSQUEEZY_API_KEY;
  if (!apiKey) {
    throw new Error("Billing is not configured");
  }
  ...
  const parsedQuantityToAdd = toInteger(quantityToAdd, 0);
  if (!targetVariantId || parsedQuantityToAdd < 0) {
  ...
  const targetQuantity = Math.max(currentQuantity + parsedQuantityToAdd, 1);
  ...
  const refreshedAttributes = refreshedData?.attributes || {};
  const refreshedVariantId = String(refreshedAttributes?.variant_id || "");
  const refreshedVariantMeta = getVariantBillingMeta(refreshedVariantId);
  const refreshedQuantity = toInteger(refreshedAttributes?.first_subscription_item?.quantity, targetQuantity);

  return {
    subscriptionId: String(refreshedData?.id || subscriptionId),
    status: String(refreshedAttributes?.status || "unknown"),
    subscriptionItemId: refreshedAttributes?.first_subscription_item?.id
      ? String(refreshedAttributes.first_subscription_item.id)
      : "",
    variantId: refreshedVariantId,
    planInterval: refreshedVariantMeta.planInterval,
    quantity: refreshedQuantity > 0 ? refreshedQuantity : targetQuantity,
    renewsAt: parseDateOrNull(refreshedAttributes?.renews_at),
    endsAt: parseDateOrNull(refreshedAttributes?.ends_at),
    currentPeriodEnd: parseDateOrNull(refreshedAttributes?.renews_at),
    portalUrl: String(refreshedAttributes?.urls?.customer_portal || ""),
    isLegacyVariant: refreshedVariantMeta.isLegacyVariant,
    migrationRequired: refreshedVariantMeta.migrationRequired
  };
}
