...
async function syncSubscriptionsFromLemonByEmail(customerEmail, organizationSlug) {
  ...
  const url = `https://api.lemonsqueezy.com/v1/subscriptions?filter[user_email]=${encodeURIComponent(normalizedEmail)}&page[size]=100`;
  ...
  for (const record of records) {
    if (record?.type !== "subscriptions" || !record?.id) {
      continue;
    }

    const attributes = record.attributes || {};
    const variantMeta = getVariantBillingMeta(
      attributes?.variant_id ?? attributes?.first_subscription_item?.variant_id ?? null
    );
    const quantity = toInteger(attributes?.first_subscription_item?.quantity, 1);
    const normalizedQuantity = quantity > 0 ? quantity : 1;
    const status = String(attributes?.status || "unknown");
    const subscriptionSet = {
      subscriptionId: String(record.id),
      status,
      variantId: String(attributes?.variant_id || ""),
      planInterval: variantMeta.planInterval,
      quantity: normalizedQuantity,
      customerEmail: String(attributes?.user_email || attributes?.customer_email || normalizedEmail).toLowerCase(),
      customerId: attributes?.customer_id ? String(attributes.customer_id) : "",
...
export async function getBillingStatusForOrganization(organization) {
  ...
  if (normalizedOwnerEmail && subscriptions.length <= 1) {
    await syncSubscriptionsFromLemonByEmail(normalizedOwnerEmail, organization?.slug);
    subscriptions = await getSubscriptionsForOrganization(organization?.slug);
  }
