...
export async function updateLemonSubscriptionFromApp({ subscriptionId, interval, quantity }) {
  const apiKey = process.env.LEMONSQUEEZY_API_KEY;
  if (!apiKey) {
    throw new Error("Billing is not configured");
  }
  ...
  const parsedQuantity = toInteger(quantity, 1);
  if (!targetVariantId || parsedQuantity < 1) {
  ...
  const refreshedQuantity = toInteger(refreshedAttributes?.first_subscription_item?.quantity, parsedQuantity);
  ...
  return {
    ...
    quantity: refreshedQuantity > 0 ? refreshedQuantity : parsedQuantity,
