...
export async function getBillingStatusForOrganization(organization) {
  ...
  if (subscriptionSummary.entitlement.length === 0 && ownerFreeSeats > 0) {
    ...
    return {
      ...
      canonicalSubscription: subscriptionSummary.canonical ?? null,
      subscriptionHistory: subscriptionSummary.history,
      hasMultipleActiveSubscriptions: subscriptionSummary.hasMultipleActive,
      canMutateSafely: false,
      billingStateMessage: "Owner access override is enabled for this organization."
    };
  }
  ...
  const primarySubscription = subscriptionSummary.canonical ?? subscriptions[0];
  ...
  return {
    ...
    canonicalSubscription: primarySubscription || null,
    subscriptionHistory: subscriptionSummary.history,
    hasMultipleActiveSubscriptions,
    canMutateSafely: subscriptionSummary.canMutateSafely,
    billingStateMessage:
      primarySubscription?.status === "cancelled" || primarySubscription?.status === "canceled"
